...
## Coding Rules
- **Schema Enforcement**: Define Pydantic models for every request/response. AI outputs must be validated against explicit structured output schemas (e.g., Pydantic parsers).
- **Abstraction Over Implementation**: Business logic must depend on abstractions (Interfaces), not concrete implementations of LLM providers, storage backends (`BaseSessionStore` with Redis/InMemory), or supermarket APIs.
- **Type Hinting**: Mandatory type hints for all function signatures.
- **Standardized Errors**: Use specific HTTP status codes and exception handlers (e.g., 422 for mapping/parsing failures, 404 for missing sessions/SKUs, 503 for downstream adapter timeouts).
